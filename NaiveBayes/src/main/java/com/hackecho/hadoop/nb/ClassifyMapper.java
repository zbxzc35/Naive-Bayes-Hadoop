...
public class ClassifyMapper extends Mapper<Text, Text, LongWritable, Text> {
    ...
    protected void setup(Context context) throws IOException {
        vocabularySize = context.getConfiguration().getLong(Main.VOCABULARY_SIZE, 100);
    ...
    public void map(Text key, Text value, Context context) throws InterruptedException, IOException {
        ...
        for (int i = 1; i < elements.length; ++i) {
            ...
            if (!trueLabels.containsKey(docId)) {
                ...
                // ASSUMPTION: The same document ID will have the same true
                // labels.
        ...
        for (Long docId : trueLabels.keySet()) {
            ...
            for (String label : wordsUnderLabel.keySet()) {
                ...
                double wordProb = (double) multiplier
                        * (Math.log(count + Main.ALPHA) - Math.log(wordLabelCount + (Main.ALPHA * vocabularySize)));
            ...
            // list of "<truelabel>:" statements for each true label of the
            // document
