...
public class ClassifyReducer extends Reducer<LongWritable, Text, LongWritable, IntWritable> {
    ...
    protected void setup(Context context) throws IOException {
        totalDocuments = context.getConfiguration().getLong(Main.TOTAL_DOCS, 100);
        uniqueLabels = context.getConfiguration().getLong(Main.UNIQUE_LABELS, 100);
    ...
    public void reduce(LongWritable key, Iterable<Text> values, Context context) throws InterruptedException,
        ...
        for (String label : probabilities.keySet()) {
            double prior = Math.log((double) docsWithLabel.get(label).intValue() + Main.ALPHA)
                    - Math.log(totalDocuments + (Main.ALPHA * uniqueLabels));
